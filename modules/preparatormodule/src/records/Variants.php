...
class Variants extends ActiveRecord
{
    ...
    {
        return '{{%preparatormodule_variants}}';
    }

    public function rules()
    {
        return [
            [['house_id', 'variant_index', 'variant_possibility_key'], 'required'],
            [['house_id', 'variant_index', 'variant_possibility_key'], 'safe'],
            ['variant_possibility_key', 'string'],
            [['house_id','variant_index'], 'integer','min' => 0],
        ];
    }

    public function getPricesOptions()
    {
        return $this->hasMany(VariantPrices::class, ['variant_id' => 'id']);
    }
}
